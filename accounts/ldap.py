...
def migrate_to_LDAP(pending_user, password):
    ...
    if (settings.LDAP_SERVER_URI
            and settings.LDAP_MANAGEMENT_DN
            and settings.LDAP_MANAGEMENT_PASSWORD
            and settings.LDAP_DEFAULT_GID
            and settings.LDAP_USERS_BASE_DN
            and settings.LDAP_DEFAULT_USER_OU
            and settings.LDAP_DEFAULT_HOME_PATH):

        connection = ldap.initialize(settings.LDAP_SERVER_URI)
        connection.bind(settings.LDAP_MANAGEMENT_DN,
